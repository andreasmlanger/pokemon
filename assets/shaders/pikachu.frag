...
vec3 sampleScene(vec2 coord)
{
    ...
    // Backdrop colour.
    vec3 c=vec3(106.0 / 255.0, 28.0 / 255.0, 154.0 / 255.0);
...
void main()
{
    ...
    coord.x = coord.x * 4 - iResolution.x * 1.5;
    coord.y = coord.y * 4 - iResolution.y * 1.5;
    coord.y = iResolution.y - coord.y;  // flip horizontally
    time = iTime;

    // Sample the scene
    fragColor.rgb = clamp(sampleScene(coord.xy), 0.0, 1.0);
